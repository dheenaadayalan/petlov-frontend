...
function EditPet({ petId }) {
  ...
  const handleDelete = async ()=>{
    if(confirm('Are you sure you want to delete you pet')==true){
        console.log('its in');
        await axios
        .delete(`http://localhost:4000/api/user/petowner/pets/delete/${petId}`,
          {
            headers: { token: userToken },
          }
        )
    ...
    <div className="">
      ...
      >
        ...
        <div className="form-check form-switch tw-flex tw-flex-row tw-justify-between tw-w-[90vw] md:tw-w-[50vw] tw-pl-0">
          <label
            className="form-check-label tw-text-quaternary tw-font-bold tw-text-xl tw-text-start "
            ...
            {oldPic.map((ele, index) => {
              return (
                <div key={index} className="carousel-item active">
                  <img src={ele} className="d-block tw-h-[35vh]" alt="..." />
                </div>
              );
            })}
          </div>
